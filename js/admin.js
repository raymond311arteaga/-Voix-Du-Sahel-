...
// abrir drawer con datos de tarjeta
document.addEventListener("click", (e)=>{
  ...
  $("#card_media").value = card.querySelector(".media img, .media video, .media iframe")?.src || "";
  $("#card_title_en").value = card.querySelector("h3").textContent;
  $("#card_desc_en").value  = card.querySelector("p").textContent;
  // dejamos FR vacío para que el admin lo traduzca si quiere
  $("#card_title_fr").value = "";
  $("#card_desc_fr").value  = "";
...
$("#saveCard").addEventListener("click", async ()=>{
  ...
  const payload = {
    mediaUrl: $("#card_media").value.trim(),
  };
  ...
  await updateDoc(doc(db,"sections",selected.section,"cards",selected.id), payload);
  drawer.classList.remove("open");
  toastMsg("Card saved.");
  render();
});

// precargar hero en el panel cuando se abre
